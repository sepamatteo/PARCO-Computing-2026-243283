...
\begin{itemize}
    \item \textbf{Parallel CSR-SpMV:} The parallelization is done over the rows of the matrix. We use an OpenMP parallel for loop with \textbf{guided} scheduling to distribute the rows among the threads.
...
Experiments used the 5 fixed matrices from Section \ref{sec:exp}:
...
\begin{center}
\adjustbox{max width=1.05\columnwidth, keepaspectratio}{%
  \includegraphics{all_matrices.pdf}
}
\captionof{figure}{Graph of all test matrices.}
...
The strong scaling results, visualized in Figure~\ref{fig:all_matrices}, reveal distinct behaviors across matrix sizes. For smaller matrices such as \texttt{1138\_bus} and \texttt{bcsstk18}, speedup remains below 1.2 and efficiency drops rapidly beyond 4 processes, with communication overhead exceeding 70\% due to disproportionate ghost exchanges relative to computation. In contrast, larger matrices such as \texttt{cage14}, \texttt{nlpkkt160}, and \texttt{Queen\_4147} exhibit stronger initial scaling, achieving peak speedups of 1.6--5.1 and GFLOPs up to 5.3 at 8 processes, with efficiencies in the range of 19--63\%. However, beyond 8 processes, performance degrades as communication fractions rise to 70--90\%, while average ghost counts stabilize or decrease. These trends highlight the limitations of the cyclic distribution in high-process-count regimes for irregular sparse matrices.
...
The generation process (implemented in \texttt{src/matrix\_gen.cpp}) works as follows:
\begin{itemize}
