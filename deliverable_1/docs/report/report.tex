...
This project focuses on the implementation and performance analysis of SpMV using the popular sparse matrix format \textbf{Compressed Sparse Row (CSR):} A more compact format that enables efficient row-wise access to the matrix elements.
...
\begin{itemize}
    \item The kernel executes approximately \textbf{95 M instructions} and approximately \textbf{44 M data references} — consistent with approximately \texttt{1.6-{}-2} memory operations per non-zero.
